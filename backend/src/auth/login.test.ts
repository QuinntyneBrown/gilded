...
function fakeMailer(): { mailer: Mailer; sent: { email: string; token: string }[] } {
  ...
  return {
    sent,
    mailer: { sendVerification: (email, token) => { sent.push({ email, token }); return Promise.resolve(); }, sendReset: () => Promise.resolve(), sendInvite: () => Promise.resolve(), sendRejection: () => Promise.resolve(), sendChosenNotification: () => Promise.resolve(), sendDeletionConfirmation: () => Promise.resolve() },
  };
}
