...
function fakeMailer(): { mailer: Mailer; sent: string[] } {
  ...
  return {
    sent,
    mailer: { sendVerification: (email) => { sent.push(email); return Promise.resolve(); }, sendReset: () => Promise.resolve(), sendInvite: () => Promise.resolve(), sendRejection: () => Promise.resolve(), sendChosenNotification: () => Promise.resolve(), sendDeletionConfirmation: () => Promise.resolve() },
  };
}
